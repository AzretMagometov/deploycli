...
- Сценарий 1: два стека с ОДИНАКОВЫМИ именами сервисов и разными доменами
  ...
  префикса пространства имён роутер и сервис в этом генераторе делят
  одно и то же имя (ADR-003) — эмпирическая проверка вручную (комментарий
  к dpc-tm3.27) показала, что в этом случае Traefik ломается ещё раньше,
  чем на объединении пулов серверов: он видит два роутера с одним именем
  и разными правилами Host(...) и роняет ОБА («Router defined multiple
  times with different configurations» в логе), оба домена отвечают 404.
  Общий пул серверов у одноимённого сервиса (риск, которым ADR-003
  мотивирует префикс) этим прогоном не наблюдался напрямую — его закрыл
  бы собой более ранний отказ роутера; сценарий всё равно красный, что и
  требовалось. Поломка не встроена сюда постоянным тестом: у генератора
  нет и не должно быть режима «без пространства имён» — воспроизвести её
  можно только временным вмешательством в сборку меток.
